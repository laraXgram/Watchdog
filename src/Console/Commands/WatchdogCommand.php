...
namespace LaraGram\Watchdog\Console\Commands;

use LaraGram\Console\Command;
use LaraGram\Process\Exceptions\ProcessTimedOutException;
use LaraGram\Watchdog\File;
use LaraGram\Watchdog\Guards\EnsurePcntlIsAvailable;
use LaraGram\Watchdog\Options;
use LaraGram\Watchdog\ProcessFactory;
...
class WatchdogCommand extends Command
{
    ...
    protected $signature = 'watchdog
        ...
        {--user= : Filter the logs by the given authenticated ID}
    ...
    public function handle(ProcessFactory $processFactory): void
    {
        ...
        $this->file = new File(storage_path('watchdog/'.uniqid().'.watchdog'));
        $this->file->create();
        ...
        try {
            $processFactory->run($this->file, $this->output, $this->laragram->basePath(), $options);
